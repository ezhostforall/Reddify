export async function fetchCommentsForPost(postId, limit = 10, after = '') {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch comments for post ID ${postId}: ${response.status}`);
  }
...
export async function fetchCommentDetails(commentId) {
  ...
  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(`Failed to fetch comment details for ID ${commentId}: ${response.status}`);
  }
  const json = await response.json();
